...
railroad_w_raw <- macrobond(c("ustran0035",	
                              "ustran0032"), class = "tbl") %>%
  ts_wide() %>% 
...
fuel_sales_w_raw <- macrobond(c("usprod0981",
                                "usprod0982",
                                "usprod0983"), class = "tbl") %>%
  ts_wide() %>%
...
mb_raw <- macrobond(c("ustrad4628",
                      # "uslama3294",
                      # "uslama3349",
                      "usprod0685"), class = "tbl") %>% 
  group_by(id) %>% 
  mutate(KW = date2ISOweek(time)) %>%
  separate(KW, c("ISOyear", "ISOweek", "day"), sep = "-") %>%
  mutate(ISOweek = as.numeric(str_remove(ISOweek, patter = "^W")),
...
us_gdp_rt <-
  data_frame(var = setdiff("gdp_us", "title")) %>%
  rowwise() %>%
  ...
  mutate(pub_date = as.Date(paste0(Year, "-", Date)), 
...
# wei_original %>% ts_c(wei_original_rt, wei_scaled_pcy
#                       ) %>% ts_plot()  
