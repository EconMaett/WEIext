# A timeDate Sequence
date.sequence <- timeSequence(as.Date("1995-01-01"), 
                              as.Date("2025-12-31"))  # a short example period with three London holidays
...
restrict_forecast <- seq.Date(from = last_week_monday + 7,
                              length.out = 53, 
                              by = "weeks") %>% 
  as_tibble() %>% 
